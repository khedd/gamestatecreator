...
public class BinarizedGraphNode {
    ...
    private BinarizedGraphNode mParent;

    private ArrayList<BinarizedGraphNode> mNodes;
     ...
     */
    boolean AddNode ( BinarizedGraphNode node){
        node.mParent = this;
    ...
    private void markCycle(BinarizedGraphNode node, int[] cycleStartEnd) {
        BinarizedGraphNode parent = node;
     ...
     */
    private void pruneParentCycleFromGraph(BinarizedGraphNode node, int cycleLength) {
        if (cycleLength != 0) {
            if (this == node) {
     ...
     */
    private int checkCycle ( ArrayList<String> userActions, int cycleStartEnd[]){
     ...
     */
    private void checkCycleParentHelperGetNames (ArrayList<String> userActions){
        if ( mParent != null){
    ...
    void print() {
        if ( mNodes.isEmpty()){
            ...
            System.out.println ( toString());
            for (BinarizedGraphNode graphNode : mNodes) {
                System.out.println("***************************************");
...
//        return null;
    ...
    private void getPaths (BinarizedGraphNode root, ArrayList<ArrayList<SimplifiedNode>> paths,
...
//                currentPath.subList(depth, currentPath.size()).clear();
        ...
        if ( root.mNodes.isEmpty()){
            ...
            for (BinarizedGraphNode graphNode : root.mNodes) {
